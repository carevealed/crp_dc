...
def parse_arg():
    ...
    parser = argparse.ArgumentParser(
        description='Harvest metadata and generate DCTERMS or DC XML'
        'Written by Brian Thomas for the California Revealed Project'
    )
    parser.add_argument(
        '-i',
    ...
    parser.add_argument(
        '-d',
        help='Full path to spreadsheet, csv and excel formats accepted'
    )
    parser.add_argument(
        '-o',
        help='Output metadata type, DCTERMS for current metadata format and DC for old metadata format'
    )
...
def main():
    # compile arguments
    print('\n- California Revealed Project Dublin Core Metadata Generator version 2 -')
    args = parse_arg()
    spreadsheet = args.d
